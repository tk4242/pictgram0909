...
## 2. Notion接続の実情と Rule Freeze（Phase 0 未完了事項）

2026-08-19時点の実測：

- 接続ワークスペース：**KKIXさんのスペース**（個人・実質空）
- クライアントのルールページ `3b9e27bff22b80558d08d2b78c7b8f77` は **404（未アクセス）**。
  共有ページ一覧・最近のページも0件
- したがって§1.1は依然「v1転記＋ユーザー確定」が根拠。**原典未凍結**

**解決手順（いずれか1つ。ユーザー対応が必要）**：
1. ルールページを「KKIXさんのスペース」へ複製する（Notion上で Duplicate to）
2. ルールページが属するワークスペースへClaudeのNotion接続を追加する
3. ルールページを tmysk76@gmail.com へ共有する

接続後、`notion-fetch` でMarkdown取得→ `rules.yaml` 化→SHA-256（v2.0 §5 Rule Versioning）。
原典と§1.1が食い違ったら **RULE_CHANGE_DETECTED として停止**し、差分を人間に提示する。
...
1. **Notionルール原典の凍結**（§2。ユーザーのワークスペース操作が必要）
2. お手本スタイルガイドの整備と、それに沿った記事品質の引き上げ（進行中）
3. Notion納品モジュール（`delivery/notion.py`）の恒久実装（今回は手動運用で先行）
